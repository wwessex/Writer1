import { Search, Moon, MoreHorizontal, Maximize2, PanelRight } from 'lucide-react';
...
export function TopBar({ projectTitle = 'Writer1 Project', onFocusMode, onSearch, onToggleInspector }: TopBarProps) {
  return (
    <header className="h-14 border-b border-white/10 bg-[#171A1D]/95 backdrop-blur supports-[backdrop-filter]:bg-[#171A1D]/80 px-4 flex items-center justify-between gap-3 shrink-0">
      ...
      <div className="flex items-center gap-2 min-w-0">
        ...
        <button className="h-9 max-w-[260px] rounded-xl px-3 bg-white/5 hover:bg-white/10 border border-white/10 flex items-center gap-2 min-w-0">
          <span className="truncate text-sm font-medium">{projectTitle}</span>
          <span className="text-xs text-[#AAB2BD]">&#9662;</span>
        </button>

        <button className="h-9 rounded-xl px-3 hover:bg-white/5 text-sm text-[#AAB2BD]">
          New
        </button>
      </div>
      ...
      <div className="flex items-center gap-2">
        <div className="h-8 rounded-lg px-2.5 border border-[#3FC17A]/20 bg-[#3FC17A]/10 text-[#B8F1D0] text-xs flex items-center">
          Saved
        </div>
